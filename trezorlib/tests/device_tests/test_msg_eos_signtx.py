...
from trezorlib import eos, messages as proto
from trezorlib.tools import parse_path
from trezorlib.eos import parse_transaction_json, parse_action
...
class TestMsgEosSignTx(TrezorTest):
    def test_eos_signtx_transfer_token(self):
        ...
        tx = parse_transaction_json(json.loads(data))
        self.client.transport.write(_tx_msg(tx))
        response = self.client.transport.read()

        assert isinstance(response, proto.ButtonRequest)
        self.client.debug.press_yes()
        self.client.transport.write(proto.ButtonAck())
        time.sleep(1)
        x = self.client.transport.read()
        assert isinstance(x, proto.EosTxActionRequest)
        actionAck = parse_action(tx.actions.pop(0))
        self.client.transport.write(actionAck)
        y = self.client.transport.read()
    ...
    def test_eos_signtx_setcontract(self):
        ...
        assert isinstance(actionResp, proto.EosSignedTx)
        assert actionResp.signature_r.hex() == "05544203cc043f3d87c114fdcb7a41769db30cc2f08993541a355c2c52c4be7f"
        assert actionResp.signature_s.hex() == "2d9f0799051e023388f1a6fb1d7a1ce489150857a5e0a005f42cf7234a0b2828"
        assert actionResp.signature_v == 32
...
def _tx_msg(tx):
    msg = proto.EosSignTx()
    msg.address_n = parse_path("m/44'/194'/0'/0/0")
    msg.chain_id = tx.chain_id
    msg.header = _tx_header(tx)
